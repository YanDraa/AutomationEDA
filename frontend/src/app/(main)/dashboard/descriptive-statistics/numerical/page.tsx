...
export default function Page() {
  ...
  const fetchStats = useCallback(async (f: File) => {
    ...
    try {
      ...
      form.append("file", f);
      const res = await fetch(`${BACKEND_URL}/api/analysis/numeric`, { method: "POST", body: form });
      if (!res.ok) throw new Error();
      const data = await res.json();
      // result.table is { columns, index, data } from sanitize_obj(df.T)
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div>
          <input id="num-upload" type="file" accept=".csv,.xlsx,.txt" className="hidden" onChange={onChange} />
          <Button size="sm" variant="outline" onClick={() => (document.getElementById("num-upload") as HTMLInputElement | null)?.click()}>
