...
// PairplotPanel component
function PairplotPanel({ numericColumns }: { numericColumns: string[] }) {
  const [chartOptions, setChartOptions] = useState<HighchartsOptions | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [insight, setInsight] = useState<string | null>(null);
  ...
  const handleGenerate = async () => {
    if (!canGenerate) {
      setError("Pair Plot membutuhkan minimal dua kolom numerik.");
      return;
    }
    const xCol = numericColumns[0];
    const yCol = numericColumns[1];
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="text-base">Pair Plot — Scatter Matrix</CardTitle>
        <CardDescription>
          Menampilkan scatter plot untuk setiap pasangan kolom numerik ({numericColumns.length} kolom terdeteksi).
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-6">
        <Button
        ...
        {!canGenerate ? (
          <p className="text-muted-foreground text-sm">
            Upload dataset dengan minimal 2 kolom numerik untuk menampilkan pair plot.
          </p>
        ) : null}
        ...
        {insightVisible && numericColumns.length >= 2 ? (
          <p className="text-muted-foreground text-xs">
            Insight AI untuk pair plot menampilkan hubungan pairwise antar kolom numerik.
          </p>
        ) : null}
      </CardContent>
    </Card>
...
// Updated Page component with chart toggle
export default function Page() {
  const [activeChart, setActiveChart] = useState<'heatmap' | 'pairplot'>('heatmap');

  return (
    <VizPageShell
      title={activeChart === 'heatmap' ? "Multivariate — Heatmap" : "Multivariate — Pair Plot"}
      description={activeChart === 'heatmap' ? "Heatmap korelasi Pearson untuk semua kolom numerik." : "Scatter matrix menampilkan semua pasangan kolom numerik."}
    >
      {({ numericColumns }) => (
        <>
          <div className="flex gap-4 mb-4">
            <Button variant={activeChart === 'heatmap' ? 'default' : 'outline'} onClick={() => setActiveChart('heatmap')}>Heatmap</Button>
            <Button variant={activeChart === 'pairplot' ? 'default' : 'outline'} onClick={() => setActiveChart('pairplot')}>Pair Plot</Button>
          </div>
          {activeChart === 'heatmap' ? (
            <HeatmapPanel numericColumns={numericColumns} />
          ) : (
            <PairplotPanel numericColumns={numericColumns} />
          )}
        </>
      )}
    </VizPageShell>
